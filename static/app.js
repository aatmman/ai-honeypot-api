...
async function startSimulation() {
    ...
    // ── Phase 2: Voice Analysis Demo ──
    await sleep(1500);
    updateProgress(50, 'Phase 2: Voice Analysis (Whisper)');
    ...
    document.getElementById('sim-voice-content').innerHTML = `
        ...
        ${waveHtml}
        <p style="margin-top:8px">⏳ Transcribing with Groq Whisper v3...</p>
    `;
    scrollFeaturePanel();
    ...
    document.getElementById('sim-voice-content').innerHTML += `
        ...
        <div style="margin-top:10px">
            ${allIntel.bankAccounts.map(v => `<span class="sim-intel-pill bank">🏦 ${v}</span>`).join('')}
            ${allIntel.upiIds.map(v => `<span class="sim-intel-pill upi">📱 ${v}</span>`).join('')}
            <span class="badge badge-high" style="margin-left:8px">⚠️ SCAM CONFIRMED</span>
        </div>
    `;
    scrollFeaturePanel();

    // ── Phase 3: Image Intelligence Demo ──
    await sleep(2500);
    updateProgress(65, 'Phase 3: Image Intelligence (Vision)');
    ...
    document.getElementById('sim-image-content').innerHTML = `
        ...
        ${scenario.imageHtml}
        <p style="margin-top:8px">⏳ Analyzing with Groq Vision AI (llama-3.2-90b)...</p>
    `;
    scrollFeaturePanel();
    ...
    document.getElementById('sim-image-content').innerHTML += `
        <div style="margin-top:12px;padding:12px;background:var(--bg-primary);border-radius:var(--radius-sm);box-shadow:var(--clay-shadow-inset)">
            <strong>🔍 Analysis Result:</strong><br>${scenario.imageAnalysis}
        </div>
        <div style="margin-top:10px">
            ${allIntel.phoneNumbers.map(v => `<span class="sim-intel-pill phone">📞 ${v}</span>`).join('')}
            ${allIntel.phishingLinks.map(v => `<span class="sim-intel-pill link">🔗 ${v}</span>`).join('')}
